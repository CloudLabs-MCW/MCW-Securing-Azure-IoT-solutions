...
2. Select the **oilwells-server-<inject key="DeploymentID" enableCopy="false"/>** virtual machine.
...
6. Open the Hyper-V manager mmc, select the **server-<inject key="DeploymentID" enableCopy="false"/>** then select the `Ubuntu` image, start it if not started.
