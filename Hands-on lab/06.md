...
1. In the Azure Portal, browse to your **iotsecurity-<inject key="DeploymentID" enableCopy="false"/>** resource group, then select the **oilwells-logging-<inject key="DeploymentID" enableCopy="false"/>** Log Analytics Workspace.
...
8. Select the **oilwells-edgevm-<inject key="DeploymentID" enableCopy="false"/>**.
...
1. Repeat the above steps you have learned to add the **oilwells-edgevm-<inject key="DeploymentID" enableCopy="false"/>** to the device provisioning service and the Iot Hub as **oilwells002**.  This would include most of the tasks in Exercise 2:
...
4. Update the script with the IP address of the **oilwells-edgevm-<inject key="DeploymentID" enableCopy="false"/>** device.  You can get the IP from the Azure Portal.
